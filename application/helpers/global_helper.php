		...
		function sendEmail($from = null, $to = null, $sub = null, $msg = null, $reply_to = null, $cc = null, $bcc = null, $attachment = null)
		{
			...
			if($msg != "") {		
				...
				elseif($default == 'default php')
				{
						...
						$headers .= 'From: '.$CI->config->item('site_settings')->site_title.'<'.$from.'>' . "\r\n";
...
if( !function_exists('share_check') ){
	...
	function share_check( $type='' ){
		$url = array(
			...
			'facebook'	=> '//facebook.com/sharer.php',
			'buzz'		=> '//www.google.com/buzz/post',
			'vkontakte'	=> '//vkontakte.ru/share.php',
			'googleplus' => '//plus.google.com/share',
	...
	function share_button( $type='', $args=array() ){
		...
		if( $type == 'twitter'){
			if( isset($args['iframe']) ){
				...
				src="//platform.twitter.com/widgets/tweet_button.html?{$param}"></iframe>
DOT;
			}else{
				foreach( explode(' ', 'url via text related count lang counturl') as $v ){
				...
				$button = <<<DOT
				<a href="//twitter.com/share" class="twitter-share-button" {$param}>Tweet</a>
...
DOT;
			...
			$button = <<<DOT
			<a title="{$args['title']}" class="google-buzz-button" href="//www.google.com/buzz/post" {$param}></a>
...
/******Tutor Avg. Rating Value ******/
if ( ! function_exists('tutorAvgRatingValue'))
{
	function tutorAvgRatingValue($tutor_id=null)
  	{
		$rating_value = 0;
		if(is_numeric($tutor_id)) 
		{
			$CI = & get_instance();
			$query = "select AVG(tr.rating) as rating_value from ".TBL_PREFIX."tutor_reviews tr WHERE tr.tutor_id=".$tutor_id." AND tr.status='Approved'";
			
			$record = $CI->db->query($query)->result();
			
			if(!empty($record))
				$rating_value = $record[0]->rating_value;
		}
		return $rating_value;
    }
}

 /******Tutor no of comments ******/
if ( ! function_exists('tutorCommentNumber'))
{
	function tutorCommentNumber($tutor_id=null)
  	{
		$no_of_comments = 0;
		if(is_numeric($tutor_id)) 
		{
			$CI = & get_instance();
			$query = "select count(tr.id) as no_of_comments from ".TBL_PREFIX."tutor_reviews tr WHERE tr.tutor_id=".$tutor_id." AND tr.status='Approved'";
			
			$record = $CI->db->query($query)->result();
			
			if(!empty($record))
				$no_of_comments = $record[0]->no_of_comments;
		}
...
/*if (! function_exists('language_type'))
{
	function language_type()
	{
		...
		if ($default_language=='') {
			...
			$default_language = $CI->config->item('site_settings')->default_language;
...
if (! function_exists('language_type'))
{
	function language_type()
	{
		...
		// this is to fix some errors with rtl languages updated on 28-11-2018 by Peter
		$class = $CI->uri->segment(1, 0);
		$method = $CI->uri->segment(2, 0);
		$param = $CI->uri->segment(3, 0);
		// echo $class;

		if(!empty($class) && ($class=='tutor' || $class='student')){

			if(!empty($method) && $method=='manage-courses'){
				if($param=='add' || $param=='edit'){
					$textdir = 'ltr';
				}

			}else if(!empty($method) && $method=='list-packages'){
				$textdir = 'ltr';
			}else if(!empty($method) && $method=='manage-locations'){
				$textdir = 'ltr';
			}else if(!empty($method) && $method=='manage-teaching-types'){
				$textdir = 'ltr';
			}
...
if ( ! function_exists('get_user_image'))
{
	...
	function get_user_image($user)
	{
		...
		if (!empty($image)) 
		{
			if (file_exists('assets/uploads/profiles/'.$image)) {
				return URL_PUBLIC_UPLOADS_PROFILES.$image;
			}
			else {
				if ($user->gender=='Male')
					return URL_UPLOADS_PROFILES_TUTOR_MALE_DEFAULT_IMAGE;
...
function update_notification($link) {

	$CI	=&	get_instance();
	$CI->load->database();

	$notifications = $CI->base_model->fetch_records_from("notifications",array('page_link'=>$link,
					'admin_read'=>0));
	...
	if (!empty($notifications)) {
		foreach ($notifications as $notification) {
			$data = array();
			$data['admin_read'] = 1;
			$data['admin_read_date'] = date('Y-m-d H:i:s');

			$CI->base_model->update_operation($data,"notifications",array('notification_id'=>$notification->notification_id));
			unset($data);
		}
	}
...
if ( ! function_exists('get_site_logo'))
{
	function get_site_logo()
	{
		...
		if($CI->config->item('site_settings')->logo!= '' && file_exists('assets/uploads/settings/'.$CI->config->item('site_settings')->logo)) 
		{
			$site_img = URL_PUBLIC_UPLOADS_SETTINGS.$CI->config->item('site_settings')->logo;
		}
		else
		{
			$site_img = URL_FRONT_IMAGES.'Logo.png';
		}
...
// {
// 	function dd($data = $_POST, $die = true)
// 	{
// 		echo '<pre>';
// 		print_r($data);
// 		if ( $die ) {
