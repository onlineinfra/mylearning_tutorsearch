...
class Auth extends MY_Controller {
	...
	function index($param = '', $param2 = '')
	{
		...
		else
		{
			...
			$user_group_arr = array('1' => 'Admin', '2' => 'Student', '3' => 'Tutor', '4' => 'Institute');
			...
			if($crud_state != 'list')
			{
				if(!empty($param))
				{
					...
					if($param == 1)
					{
						$title = get_languageword('admins');
						$crud->set_subject(get_languageword('admin'));
					}
					if($param == 2)
					{
	...
	function callback_field_phone_code($val = '', $primary_key = null)
	{
		...
		if(!empty($countries)) {
			foreach ($countries as $key => $value) {
				$country_opts[$value->phonecode] = $value->nicename." +".$value->phonecode;
			}
		}
		...
		if($this->base_model->update_operation($up_data, 'users', array('id' => $user_id))) {
			...
			if(!empty($email_tpl)) {
				...
				if(!empty($email_tpl->template_content)) {
					...
					$original_vars  = array($logo_img, $site_title, $user_rec->username, $approvemnet_status, $extra_txt.'<a href="'.URL_AUTH_LOGIN.'">'.get_languageword('Login Here').'</a>');
					$temp_vars		= array('__SITE_LOGO__', '__SITE_TITLE__', '__USER__NAME__', '__APPROVEMENT_STATUS__', '__LOGINLINK__');
	...
	*/
	...
	{		
		...
		//Code for Create an Account Button
